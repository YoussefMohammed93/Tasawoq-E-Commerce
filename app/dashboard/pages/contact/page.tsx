...
export default function ContactPage() {
  ...
  // Define the type for contact page data
  ...
  // Update form data when contact page data is loaded
  useEffect(() => {
  ...
  const handleChange = (
    ...
    // Handle nested properties (mapLocation)
    if (name.includes(".")) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Filter out Convex system fields before submitting
      const { _id, _creationTime, ...contactPageData } = formData;

      // Only include fields that have been modified
      const updates: any = {
        mapLocation: contactPageData.mapLocation, // Always include mapLocation since it's required
      };

      // Compare with original data and only include changed fields
      if (contactPageData) {
        Object.entries(contactPageData).forEach(([key, value]) => {
          if (key === "mapLocation") return; // Skip mapLocation as we've already added it

          // Include the field if it has a value (even empty string)
          if (value !== undefined) {
            updates[key] = value;
          }
        });
