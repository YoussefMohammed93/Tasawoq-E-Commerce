...
export function FooterSectionItem({
  ...
  // State for tracking title changes
  ...
  // We'll check if title has changed directly in the onChange handler
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    // Only update if the value is different from the original
                    if (newValue !== originalTitle) {
                      // Update the UI but don't save to database yet
                      // The save button will handle saving to the database
                      onUpdate(index, {
                        title: newValue,
                        links: section.links,
                      });
            ...
            <DndContext
              ...
              onDragEnd={async (event) => {
                ...
                if (over && active.id !== over.id) {
                  ...
                  // Make sure the order property is explicitly set for each link
                  const reorderedLinks = newLinks.map((link, idx) => ({
                    ...link,
                    order: idx, // Explicitly set the order
                  }));

                  // Add a special flag to indicate this is a reordering operation
                  // This will be used in handleUpdateFooterSection to detect reordering
                  const dataWithFlag = {
                    title: section.title,
                  ...
                  // Update the UI and save to database immediately
                  ...
                  console.log("Reordering links:", {
                    oldIndex,
                    newIndex,
                    reorderedLinks: reorderedLinks.map((l) => l.order),
                  });
                }
              }}
              modifiers={[]}
