...
export function SocialLinkItem({
  ...
  // Reset preview image when link changes from parent
  useEffect(() => {
  ...
  // Get image URL if it's a storage ID
  ...
  const getImageSource = () => {
    // If we have a preview image (from recent upload), use that
    if (previewImage) {
      return previewImage;
    }
    // If there's a preview image in the link object (from parent component)
    else if (link._previewImage) {
      return link._previewImage;
    }
    // If it's a storage ID and we have the URL
    else if (
      link.image &&
      typeof link.image === "string" &&
      (link.image.startsWith(":") ||
    ...
    // If it's a URL starting with http
    else if (
      link.image &&
      typeof link.image === "string" &&
      link.image.startsWith("http")
    ...
    // If it's a relative path to a public file
    else if (
      link.image &&
      typeof link.image === "string" &&
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Pass the file to the parent component for handling
    onImageUpdate(index, file);
  };

  const handleDeleteClick = () => {
    // Mark for deletion in the parent component
    onDelete(index);
    // No need to set local state as we're not showing a save button anymore
  };
