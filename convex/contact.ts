...
export const getContactPage = query({
  handler: async (ctx) => {
    ...
    return contactPage || {}; // Return empty object instead of defaults
  },
});
...
export const saveContactPage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      ...
      // For new entries, we need to ensure all required fields are present
      ...
      // Only use defaults for fields that weren't provided
      const completeData = {
        ...defaultContactPage,
        ...args,
      };
